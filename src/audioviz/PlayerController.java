...
public class PlayerController implements Initializable {
    ...
    private void openMedia(File file) {
        ...
        timeSlider.setOnMousePressed(new EventHandler<MouseEvent>() {
                    ...
                    public void handle(MouseEvent event) {
                       mediaPlayer.pause();
                       currentText.setText(Double.toString(Math.round(timeSlider.getValue()*10.0)/10.0));
                    }
        });
    ...
    private void handleReady() {
        ...
        Double durationX = Math.round(duration.toSeconds()*10.0)/10.0;
        lengthText.setText(durationX.toString());
        Duration ct = mediaPlayer.getCurrentTime();
        Double ctX = Math.round(ct.toSeconds()*10.0)/10.0;
        currentText.setText(ctX.toString());
        currentVisualizer.start(numBands, vizPane);
        timeSlider.setMin(0);
    ...
    private void handleUpdate(double timestamp, double duration, float[] magnitudes, float[] phases) {
        Duration ct = mediaPlayer.getCurrentTime();
        double ss = ct.toSeconds();
        currentText.setText(Double.toString(Math.round(ss*10.0)/10.0));
        timeSlider.setValue(ss);
        
        currentVisualizer.update(timestamp, duration, magnitudes, phases);
    }
    
    @FXML
    private void handleMousePressed(ActionEvent event) {
        mediaPlayer.stop();
    }
    
    @FXML
    private void handleMouseReleased(ActionEvent event) {
        mediaPlayer.stop();
    }
    @FXML
